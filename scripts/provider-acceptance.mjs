...
export function runProviderAcceptance(options = {}) {
  ...
  try {
    ...
    const workspaceMarker = acceptanceCanary(nonce, "workspace");
    const isolatedHome = path.join(tempRoot, "home");
    const codexHome = path.join(tempRoot, "codex-home");
    const marketplaceRoot = path.join(tempRoot, "marketplace");
    const workspace = path.join(tempRoot, workspaceMarker);
    ...
    if (report.checks.installedHookFound) {
      ...
      for (const payload of hookPayloads) {
        const hookResult = runBounded(runner, {
          ...
          args: [path.join(installedPluginRoot, "scripts", "hook.mjs")],
          cwd: workspace,
          env: hookEnv,
        ...
        try {
          validHookOutput = isRecord(JSON.parse(hookResult.stdout));
        } catch {
          validHookOutput = false;
        }
